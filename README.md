...
| 言語・音量 | タイトルの「Options」(戦闘中は START → Options) | |

**初回起動時は英語で始まります。** 日本語にするには、タイトル画面で「Options」→「Language」を「日本語」に変更してください (設定は保存されます)。
...
- **v1.0.1** — Pyxel Web (ブラウザ / PWF) で起動直後に落ちる問題を修正
- **v1.0.2** — BGM を軽量化 (51MB → 18MB)。メモリの少ない Web 環境でも起動できるように
- **v1.1.0 (予定)** — 後編 (アリア編)
